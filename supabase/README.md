...
The `video-files` migration sets the bucket-level limit to 250 MB. Supabase Pro projects can raise the global Storage file size limit above this value in Storage Settings, and the per-bucket limit keeps the application policy at 250 MB.
...
The admin dashboard calculates available Storage capacity from the Storage API object sizes and `SUPABASE_STORAGE_QUOTA_GB`. If the variable is omitted, it uses the Supabase Pro Storage Size quota of 100 GB as the baseline.
