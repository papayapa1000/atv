...
export function DepositAccountGuide({ compact = false, headingLevel = "h2", layout = "stacked" }: DepositAccountGuideProps) {
      ...
      <div className={isHorizontal ? "grid gap-5 lg:grid-cols-[minmax(16rem,0.75fr)_minmax(0,1.25fr)] lg:items-start" : undefined}>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className={`${compact ? "mt-2 text-xs" : "mt-3 text-sm"} leading-6 text-ink-muted`}>ATV와 수상레저 계좌가 다릅니다. 예약글을 남기기 전에 선택한 종목을 기준으로 확인해 주세요.</p>
          </div>
        </div>

        <div className={`grid gap-3 ${isHorizontal ? "" : "mt-5"} ${compact ? "" : "lg:grid-cols-2"}`}>
              ...
              <article key={account.key} className={`grid h-full grid-rows-[auto_1fr_auto] border px-4 py-4 ${isAtv ? "border-sun/35 bg-sun/10" : "border-lake/18 bg-foam"}`}>
                ...
                <dl className="mt-4 grid border-t border-foreground/10 pt-2 text-sm">
                  ...
                  <div className="grid min-h-10 grid-cols-[4.5rem_minmax(0,1fr)] items-center gap-3 py-2">
                    ...
                    <dd className="numeric break-all text-base font-extrabold text-foreground sm:break-normal sm:whitespace-nowrap">{account.accountNumber}</dd>
                  </div>
                </dl>
              </article>
            );
