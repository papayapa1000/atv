...
type AdminVideoPostEditModalProps = {
  post: VideoPost;
};
...
function AdminVideoEditFormActions({ onCancel }: AdminVideoEditFormActionsProps) {
  ...
  return (
    <>
      {pending ? (
        <div role="status" aria-live="polite" className="flex items-center gap-3 border border-lake/18 bg-lake/6 px-4 py-3 text-sm font-bold text-lake">
          <CircleNotch aria-hidden="true" className="h-4 w-4 animate-spin" weight="bold" />
          영상 업로드 중입니다. 완료될 때까지 기다려 주세요.
        </div>
      ) : null}
      <div className="flex flex-wrap justify-end gap-2 border-t border-foreground/10 pt-4">
        <button
          ...
          onClick={onCancel}
          disabled={pending}
          className="spring inline-flex items-center justify-center border border-foreground/14 px-4 py-2.5 text-sm font-bold text-foreground/64 hover:border-foreground hover:text-foreground disabled:cursor-not-allowed disabled:opacity-55"
        >
          취소
        </button>
        <button
          type="submit"
          disabled={pending}
          className="spring inline-flex items-center justify-center gap-2 border border-lake bg-lake px-4 py-2.5 text-sm font-bold text-white hover:border-foreground hover:bg-foreground disabled:cursor-not-allowed disabled:opacity-55"
        >
          {pending ? (
            <CircleNotch aria-hidden="true" className="h-4 w-4 animate-spin" weight="bold" />
          ) : (
...
export function getVideoFileName(videoUrl: string | null) {
  if (!videoUrl) {
    return "";
  }

  const withoutHash = videoUrl.split("#")[0] ?? "";
          ...
          <div role="dialog" aria-modal="true" aria-labelledby={titleId} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto border border-foreground/14 bg-white p-5 shadow-[0_32px_80px_-44px_rgba(16,34,30,0.55)] sm:p-6">
            <div className="flex items-start justify-between gap-4 border-b border-foreground/10 pb-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-foreground/58">유튜브 링크를 입력하거나 새 영상 파일을 첨부하면 영상 소스를 교체합니다.</p>
              </div>
              <button
                type="button"
            ...
            <form action={updateAdminVideoPostAction} className="mt-5 grid gap-4">
              ...
              <label className="block">
                <span className="text-sm font-bold text-foreground">유튜브 링크</span>
                <input
                  name="youtubeUrl"
              ...
              <AdminVideoFileReplacementField currentFileName={getVideoFileName(post.videoUrl)} />
              <label className="block">
