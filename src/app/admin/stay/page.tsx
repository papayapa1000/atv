...
import { listAdminStayPosts, type StayPost } from "@/lib/stay/repository";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "주변 숙박 관리 | 제천 수상레저 & 청풍 ATV",
  robots: {
    index: false,
    follow: false,
  },
...
export default async function AdminStayPage({ searchParams }: AdminStayPageProps) {
  ...
  try {
    posts = await listAdminStayPosts();
  } catch {
    loadError = "주변 숙박 목록을 불러오지 못했습니다. Supabase 설정과 stay_posts 테이블을 확인해 주세요.";
  }

  return (
    <main className="depth-mint min-h-screen bg-foam text-foreground">
      <AdminTopbar active="stay" />
      <section className="px-5 py-12 lg:px-8 lg:py-16">
        <div className="mx-auto max-w-[1440px]">
          <div className="flex flex-col gap-4 border-b border-foreground/12 pb-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex border-b border-foreground/18 pb-2 text-xs font-semibold uppercase text-lake">Stay Admin</p>
              <h1 className="mt-5 text-3xl font-semibold leading-tight sm:text-5xl">주변 숙박 관리</h1>
              <p className="mt-4 max-w-2xl text-base leading-8 text-ink-muted">
                숙박 목록과 상세페이지에 표시할 제목, 가격, 이미지, 본문을 등록합니다.
              </p>
            </div>
            <Link
              href="/stay"
          ...
          <div className="mt-8 grid gap-8 xl:grid-cols-[28rem_1fr]">
            <aside className="h-fit border border-foreground/12 bg-surface p-5 sm:p-6">
              <h2 className="text-xl font-semibold">새 숙박 정보 등록</h2>
              <p className="mt-3 text-sm leading-7 text-foreground/58">등록 후 공개 상태라면 숙박 목록과 상세페이지에 바로 표시됩니다.</p>
              <div className="mt-6">
                <AdminStayPostForm />
              </div>
            </aside>
            ...
            <section className="min-h-[36rem] border border-foreground/12 bg-surface p-4 sm:p-5">
              <div className="flex items-center justify-between gap-4 border-b border-foreground/10 pb-4">
                <h2 className="text-xl font-semibold">등록된 숙박 정보</h2>
              ...
              {posts.length > 0 ? (
                    ...
                    return (
                      <article key={post.id} className="border border-foreground/12 bg-white">
                        <div className="relative aspect-[16/10] overflow-hidden bg-mist">
                          {coverImageUrl ? (
                            <Image src={coverImageUrl} alt={post.title} fill sizes="(min-width: 1280px) 28vw, 50vw" className="object-cover" />
                          ) : (
                        ...
                        <div className="p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <h3 className="truncate text-base font-bold leading-6">{post.title}</h3>
                              <p className="numeric mt-1 text-sm font-extrabold text-lake">{post.price}</p>
                            </div>
                            <span
                              className={`inline-flex shrink-0 items-center gap-1 border px-2 py-1 text-[0.68rem] font-bold ${
                                post.isPublished ? "border-lake/24 bg-lake/10 text-lake" : "border-foreground/12 bg-surface-muted text-foreground/48"
                              }`}
                            >
                              {post.isPublished ? (
                ...
                <div className="flex min-h-[30rem] flex-col items-center justify-center px-5 py-16 text-center">
                  <p className="text-lg font-semibold">등록된 숙박 정보가 없습니다.</p>
                  <p className="mt-3 text-sm leading-7 text-ink-muted">왼쪽 등록 영역에서 첫 숙박 정보를 추가해 주세요.</p>
                </div>
              )}
            </section>
          </div>
