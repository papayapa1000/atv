...
test("reservation write phone card uses a warm accent background", () => {
  const pageSource = readFileSync("src/app/reservation/write/page.tsx", "utf8");

  assert.equal(pageSource.includes("border border-foreground/12 bg-surface p-5 sm:p-6"), false);
  assert.equal(
    pageSource.includes(
      "border border-sun/30 bg-sun/12 p-5 text-foreground shadow-[0_16px_30px_-24px_rgba(185,79,49,0.62)] sm:p-6",
    ),
    true,
  );
  assert.equal(pageSource.includes('className="text-sm font-bold text-sunset"'), true);
});
